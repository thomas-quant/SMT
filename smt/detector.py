...
def _find_valid_swing_low(df: pd.DataFrame) -> Optional[Dict[str, Any]]:
    """
    Find the most recent valid swing low within df.
...
def _find_valid_swing_high(df: pd.DataFrame) -> Optional[Dict[str, Any]]:
    """
    Find the most recent valid swing high within df.
...
def check_swing_smt(
    ...
    def _check_swing_direction(
        ...
        swing_a1 = swing_finder(data_a1)
        swing_a2 = swing_finder(data_a2)
...
def check_fvg_smt(
    ...
    Args:
        timestamp_tolerance: Allow FVG timestamps to differ by this many candles.
                           Default 1 matches the swing SMT behaviour.
    """
    # Validate dataframes
    if not _validate_dataframes(df_a1, df_a2, min_len=lookback_period + 1):
        return None
    ...
    # Both FVGs must be within tolerance (measured in candle count)
    idx_a1 = _get_timestamp_index(df_a1, fvg_a1['timestamp'])
    idx_a2 = _get_timestamp_index(df_a2, fvg_a2['timestamp'])
    if idx_a1 is None or idx_a2 is None or abs(idx_a1 - idx_a2) > timestamp_tolerance:
        return None
